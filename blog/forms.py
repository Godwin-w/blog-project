...
class RegisterForm(forms.ModelForm):
    ...
    password_confirm=forms.CharField(label="Condirm password",max_length=100 ,required=True)
...
class PostForm(forms .ModelForm):
    ...
    def clean(self):
        ...
        if title and len(title) <5:
            raise forms.ValidationError("Title must be least 5 Characters long")
        
        if content and len(content) <10:
            raise forms.ValidationError("Content must be least 10 Characters long")
    def save(self, commit = ...):
       ...
       if cleaned_data.get("img_url"):
           ...
           img_url = "posts/images/No_image_available.svg.webp"
